...
int main(int argc, char **argv){
	...
	for(i=5; i!=0; --i){
		...
		list_add(&(tmp->list2), &(mylist.list2));
 ...
 * 	 	 */
	 ...
	 */
	printf("traversing the list2 using list_for_each_entry()\n");
	list_for_each_entry(tmp, &mylist.list2, list2) {
		 printf("to= %d from= %d\n", tmp->to, tmp->from);
